  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, creator_id, title, description, short_description, cover_image_url, asset_url, asset_file_path, trailer_url, project_id, godot_version, license, version, screenshots, status, tags, category, price, created_at, updated_at, creators(id, username, display_name, profile_image_url)')
