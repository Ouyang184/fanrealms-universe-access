    ...
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {creators.length > 0 ? (
          ...
          // Fallback for when there are no creators in the database
          [1, 2, 3].map((i) => (
            <Card key={i} className="bg-gray-900 border-gray-800 overflow-hidden">
              <div className="h-32 bg-gradient-to-r from-purple-900 to-blue-900" />
              <CardContent className="pt-0 -mt-12 p-6">
                <div className="flex justify-between items-start">
                  <Avatar className="h-20 w-20 border-4 border-gray-900">
                    <AvatarImage src={`/placeholder.svg?height=80&width=80&text=${i}`} />
                    <AvatarFallback className="bg-gray-800 text-xl">{`C${i}`}</AvatarFallback>
                  </Avatar>
                  <Badge className="mt-2 bg-purple-600 flex items-center gap-1">
                    <Award className="h-3 w-3" /> Featured
                  </Badge>
                </div>
                <h3 className="text-xl font-bold mt-4">Example Creator {i}</h3>
                <p className="text-gray-400 text-sm mt-1">Join us and start creating content!</p>
