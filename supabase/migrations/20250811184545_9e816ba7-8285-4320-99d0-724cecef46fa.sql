-- Tighten public access and add safe RPCs for follows and ratings
...
-- 2) Add restricted SELECT policies for own rows only (to keep internal checks working)
DO $$ BEGIN
  -- Enable RLS just in case (should already be enabled)
...
CREATE POLICY IF NOT EXISTS "Users can view their own ratings"
ON public.creator_ratings
FOR SELECT
USING (user_id = auth.uid());

-- 3) Safe, rate-limited style RPCs (with hard caps) to expose public-safe fields only

-- a) List creators a user follows (public-safe fields)
...
-- b) List followers of a creator (public-safe fields, no timestamps)
...
-- c) Public-safe ratings for a creator (rounded timestamps, limited fields)
