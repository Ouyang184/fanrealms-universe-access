...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'switch_tier') {
      console.log('[SimpleSubscriptions] Switching tier for subscription:', subscriptionId, 'to tier:', tierId);
      
      const { currentSubscriptionId, newTierId } = await req.json();
      ...
      // Cancel current subscription immediately (no period end)
      await stripe.subscriptions.cancel(currentSub.stripe_subscription_id);

      // Update current subscription status to canceled
      await supabase
        .from('user_subscriptions')
        .update({ 
          status: 'canceled',
      ...
      // Create new checkout session with proration
      const checkoutSession = await stripe.checkout.sessions.create({
        ...
        subscription_data: {
          ...
          metadata: {
            ...
            tier_id: newTierId,
            switched_from: currentSub.tier_id
          }
        },
        success_url: `${req.headers.get('origin')}/creator/${creatorId}?success=tier_switch`,
      ...
      console.log('[SimpleSubscriptions] Successfully created switch checkout session');
